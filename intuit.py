...
class LinearConstant(LinearFormula):
    def __init__(self):
        self.sym = None

    def __str__(self):
        return f"{self.__class__.sym}"

class UnaryLinearFormula(LinearFormula):
    def __init__(self, a):
        assert(isinstance(a, LinearFormula))
        self.operand = a
        self.sym = None
        self.llprover_sym = None

    def __str__(self):
        return f"{self.__class__.sym}({self.operand})"

class BinaryLinearFormula(LinearFormula):
    def __init__(self, a ,b):
        assert(isinstance(a, LinearFormula))
        assert(isinstance(b, LinearFormula))
        self.left = a
        self.right = b
        self.sym = None
    ...
    def __str__(self):
        l = str(self.left)
        r = str(self.right)
        if isinstance(self.left, BinaryLinearFormula):
            l = f"({l})"
        if isinstance(self.right, BinaryLinearFormula):
            r = f"({r})"
        return f"{l} {self.__class__.sym} {r}"

def BaseTy(name):
    class BaseTyClass(LinearConstant):
        sym = name
    return BaseTyClass()

class Top(LinearConstant):
    sym = "top"
class Bot(LinearConstant):
    sym = "bot"
class One(LinearConstant):
    sym = "1"
class Zero(LinearConstant):
    sym = "0"

class Negation(UnaryLinearFormula):
    sym = "~"
class Exponential(UnaryLinearFormula):
    sym = "!"

class LinearFunction(BinaryLinearFormula):
    sym = "->"
class AConjunction(BinaryLinearFormula):
    sym = "/\\"
class ADisjunction(BinaryLinearFormula):
    sym = "\\/"
class MDisjunction(BinaryLinearFormula):
    sym = "+"
class MConjunction(BinaryLinearFormula):
    sym = "*"

class Sequent:
    def __init__(self, ant, cons):
        assert(isinstance(ant, LinearFormula))
        assert(isinstance(cons, LinearFormula))
        self.antecedent = ant
...
def linearProve(sequent):
    sequent_string = str(sequent) + "."
    task = subprocess.Popen(
        f"echo \"{sequent_string}\" | swipl -f llprover.pl -g ll_prover,halt",
        shell=True, stdout=subprocess.PIPE)
    data = str(task.stdout.read())
    # Look for success/fail string in the solver's output
    return data.find("Fail") == -1
...
if __name__ == "__main__":
    ...
    b = BaseTy("b")
    print(b)
    c = LinearFunction(b, MConjunction(b, b))
    print(c)
    print(linearProve(Sequent(a, c)))
